...
class QueriesController {
	...
	find(query, options, sort, limit, skip) {
		...
		return self.prepareQuery(query).then(function(query){
			...
			var ACLQuery = new Query(self.userId, self.roles, self.useMasterKey);
			query = ACLQuery.readQuery(query);
			return new Promise(function(resolve, reject){

				var c = self.db.collection(self.className).find(query, options).sort(sort).skip(skip).limit(limit);
				if (self.isCount) {
	...
	prepareQuery(query) {
		...
		Object.keys(query).forEach(function(key){
			var component = query[key];
			if (_.isObject(component) || key == "$relatedTo") {
				// {"$inQuery":
				// 	{"where":
				// 		{"image":{"$exists":true}
				// 	},
	...
	resolveIncludes(objects) {
		var self = this;
		if (_.isUndefined(self.includes) || self.includes.length === 0) {
			return Promise.resolve(objects);
		}
		// TODO: dot notation for includes;
		var fetches = {};
		var includes = this.includes;
		var includesForClass = {};
		objects.forEach(function(object){
			includes.forEach(function(key){
				var keys = key.split(".");
				key = keys[0];
				var pointer = object[key];
				if (_.isObject(pointer) && pointer.className && pointer.objectId) {
					var list = fetches[pointer.className] || [];
					list.push(pointer.objectId);
					fetches[pointer.className] = list;
					if (keys.length > 1) {
						includesForClass[pointer.className] = keys.slice(1, keys.length).join(".");
					}
				}
			});
		});

		var classes = Object.keys(fetches);

		var queries = classes.map(function(className){
			var ctrl = new QueriesController(className, self.state);
			ctrl.include(includesForClass[className]);
			ctrl.useMasterKey = self.useMasterKey;
			var ors = fetches[className].map(function(objectId){ 
		...
		.then(function(results){
			...
			results.forEach(function(result, i){
				...
				result.forEach(function(o){
					objectMap[className][o.objectId] = o;
				});
			});
			...
			objects.forEach(function(object){
				includes.forEach(function(key){
					key = key.split(".")[0];
					var pointer = object[key];
					var foundObject = objectMap[pointer.className][pointer.objectId];
	...
	insert(query, object) {
		var collection = this.db.collection(this.className);
		return new Promise(function(resolve, reject){
	...
	update (query, object) {
		...
		var ACLQuery = new Query(this.userId, this.roles, this.useMasterKey);
